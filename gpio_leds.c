...
//Vitesse de balayage coomme parametre au chargement du module
static int param;
...
static int leds_init(void)
{
    ...
    int ret_val = 0;
    printk(KERN_DEBUG "le parametre =%d\n", param);
	printk(KERN_INFO "Initializing Leds!\n");
    pr_info("Initializing the Ensea LEDs module\n");
	ensea_p = proc_mkdir("ensea",NULL);
	if(!ensea_p)
		{
		printk(KERN_INFO "Error creating proc entry");
		return -ENOMEM;
		}
	proc_file_entry = proc_create("speed", 0, ensea_p, &proc_speed);
    proc_file_entry = proc_create("dir", 0, ensea_p, &proc_dir);
...
static ssize_t leds_read(struct file *file, char *buffer, size_t len, loff_t *offset)
{
    ...
    */
    ...
    success = copy_to_user(buffer, msg, len);
...
static ssize_t leds_write(struct file *file, const char *buffer, size_t len, loff_t *offset)
{
    ...
    */
    ...
    success = copy_from_user(msg, buffer, len);
    ...
    if(success != 0) {
		...
		printk(KERN_INFO "Nouveau pattern : %d\n", pattern);
    }
...
ssize_t SPEED_read(struct file *file, char __user * buffer,size_t count, loff_t * ppos) {
	...
	if ((copy=copy_to_user(buffer, msg, count)))
		errno = -EFAULT;
	printk(KERN_INFO "Message Lu, %d, %p\n", copy, buffer);
	*ppos += count;
    return count;
}
...
ssize_t Dir_write(struct file * file, const char __user * buffer,size_t count, loff_t * ppos) {
	...
	printk(KERN_INFO "Nouvelle direction : %d\n", dir);
	return count;
}
...
static void leds_exit(void)
{
	...
	remove_proc_entry("speed", ensea_p);
	remove_proc_entry("dir", ensea_p);
